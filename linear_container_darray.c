...
static Ret linear_container_darray_delete(LinearContainer* thiz, size_t index)
{
	...
	return darray_delete(thiz->darray, index);
}
...
static size_t linear_container_length(LinearContainer* thiz)
{
	PrivInfo* priv = (PrivInfo*)thiz->priv;
...
static int linear_container_darray_find(LinearContainer* thiz, CompareFunc cmp, void* ctx)
{
	PrivInfo* priv = (PrivInfo*)thiz->priv;

	return darray_find(thiz, cmp, ctx);
}

static Ret linear_container_darray_foreach(LinearContainer* thiz, VisitFunc visit, void* ctx)
{
	PrivInfo* priv = (PrivInfo*)thiz->priv;

	return darray_foreach(thiz, visit, ctx);		
}
